...
export default function StepForm({storeId ,slugDomain , customization={}}) {
    const currentStep = useSelector((store) => store.checkout.currentStep);
    function renderFormByStep(step){
        if(step===1){
            return <PersonalDetailsForm storeId={storeId} customization={customization} />
        }else if(step === 2){
            return <ShippingDetailsForm customization={customization}/>
        }else if(step === 3){
            return <PaymentMethodForm customization={customization} />
        }else if (step === 4){
            return <OrderSummary slugDomain={slugDomain}  customization={customization}/>
        }
    }
  return (
    <div>
