...
//                    focus:text-gray-900 ${step.number === currentStep ? "text-lime-400":""} `}
//                 >
//                   {" "}
//                   {step.title}
//                 </p>
...
export default function Steps({steps, slugDomain}) {
  ...
  const cartItems = useSelector((store) => store.cart)
  return (
    <nav className="flex text-sm md:text-xl mb-8" dir="rtl">
    <ol
      role="list"
      className="flex flex-wrap gap-y-5 md:gap-y-0 items-center gap-x-1.5"
    >
      ...
      {steps.map((step, i) => {
        return (
          <li key={i}>
            <div className="flex items-center">
              <ChevronLeft className="flex-shrink-0 w-4 h-4 text-gray-400" />
              <div className="-m-1">
                <p
                  className={`p-1 ml-1.5 text-sm md:text-base font-medium text-gray-500 rounded-md focus:outline-none focus:ring-2
                   focus:text-gray-900 ${step.number === currentStep ? "text-lime-400" : ""} `}
